...
proc init_gui { IPINST } {
  ...
  set M_LFSR [ipgui::add_param $IPINST -name "M_LFSR" -parent ${Page_0}]
  set_property tooltip {enable the master LFSR} ${M_LFSR}
  set S_LFSR [ipgui::add_param $IPINST -name "S_LFSR" -parent ${Page_0}]
  set_property tooltip {enable the slave LFSR} ${S_LFSR}
  set M_SEED [ipgui::add_param $IPINST -name "M_SEED" -parent ${Page_0}]
  set_property tooltip {Seed for the master LFSR} ${M_SEED}
...
proc update_PARAM_VALUE.M_LFSR { PARAM_VALUE.M_LFSR } {
	# Procedure called to update M_LFSR when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.M_LFSR { PARAM_VALUE.M_LFSR } {
	# Procedure called to validate M_LFSR
	return true
}

proc update_PARAM_VALUE.M_SEED { PARAM_VALUE.M_SEED } {
	# Procedure called to update M_SEED when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.M_SEED { PARAM_VALUE.M_SEED } {
	# Procedure called to validate M_SEED
	return true
}
...
proc update_MODELPARAM_VALUE.M_LFSR { MODELPARAM_VALUE.M_LFSR PARAM_VALUE.M_LFSR } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.M_LFSR}] ${MODELPARAM_VALUE.M_LFSR}
}
...
proc update_MODELPARAM_VALUE.M_SEED { MODELPARAM_VALUE.M_SEED PARAM_VALUE.M_SEED } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.M_SEED}] ${MODELPARAM_VALUE.M_SEED}
}
